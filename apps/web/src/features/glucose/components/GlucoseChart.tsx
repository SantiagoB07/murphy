...
// TODO: Migrate to Convex GlucoseRecord type once views are updated
// For now, keep compatibility with legacy Glucometry format
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-card/95 backdrop-blur-sm p-3 rounded-hig border border-border/50 elevation-2">
          ...
          {data.notes && (
            <p className="text-hig-xs text-muted-foreground">{data.notes}</p>
          )}
        </div>
      )
    }
