...
const GLUCOMETRY_LABELS: Record<string, string> = {
  before_breakfast: "Antes Desayuno",
  after_breakfast: "Después Desayuno",
  before_lunch: "Antes Almuerzo",
  after_lunch: "Después Almuerzo",
  before_dinner: "Antes Cena",
  after_dinner: "Después Cena",
}
...
export function GlucoseChart({
  ...
  const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ payload: { date: string; time: string; value: number; type: string } }> }) => {
    if (active && payload && payload.length) {
      const tooltipData = payload[0].payload
      const isHigh = tooltipData.value > 180
      const isLow = tooltipData.value < 70

      return (
        <div className="bg-card/95 backdrop-blur-sm p-3 rounded-xl border border-border/50 shadow-lg">
          ...
          >
            {tooltipData.value} mg/dL
          </p>
          <p className="text-xs text-muted-foreground">
            {GLUCOMETRY_LABELS[tooltipData.type] || tooltipData.type}
          </p>
        </div>
      )
    }
  ...
  return (
    ...
    >
      ...
      <div
        className="h-64"
        role="img"
        aria-label="Gráfico de tendencia de glucosa"
      >
