...
export function SecuritySheet({ open, onOpenChange }: SecuritySheetProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      // Mock password update - will be replaced with Clerk
      await new Promise((resolve) => setTimeout(resolve, 1000))
      console.log("Updating password:", data)
      toast.success("Contrasena actualizada", {
        description: "Tu contrasena se ha cambiado correctamente.",
      })
      form.reset()
      onOpenChange(false)
    } catch (error) {
      ...
      toast.error("Error", {
        description: "No se pudo actualizar la contrasena",
      })
    } finally {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-md overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            Gestiona tu contrasena y sesiones activas
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="space-y-6 mt-6">
          ...
          <div>
            <h3 className="text-sm font-medium mb-4">Cambiar contrasena</h3>
            <Form {...form}>
