...
export function GlucoseLogSheet({
  ...
  const handleViewHistory = () => {
    onOpenChange(false)
    router.push("/glucometrias")
  }

  // Count completed slots
  const completedCount = MEAL_TIME_SLOTS.filter((slot) =>
    todayRecords.has(slot.type)
  ).length
  ...
  return (
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent
          side="bottom"
          className="h-[85vh] rounded-t-xl bg-card border-border/50 px-4 pb-8"
        >
          ...
          <div
            ...
            role="list"
            aria-label="Momentos del dia para registro"
          >
            {MEAL_TIME_SLOTS.map((slot, index) => (
              ...
              >
                <GlucoseSlotCard
                  ...
                  iconName={slot.icon}
                  onClick={() =>
                    handleSlotClick(slot.type, todayRecords.get(slot.type))
                  }
                />
              </div>
            ))}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-card via-card to-transparent pt-8">
            <Button
              ...
              aria-label="Ver historial completo de glucometrias"
            >
      ...
      {selectedSlot && (
        <DailyLogInputDialog
          open={!!selectedSlot}
          onOpenChange={(isOpen) => !isOpen && setSelectedSlot(null)}
          type="glucose"
