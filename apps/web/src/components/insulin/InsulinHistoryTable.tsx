    ...
    return {
      ...
      freqChange: previousRecord
        ? record.timesPerDay !== previousRecord.timesPerDay
          ? `${previousRecord.timesPerDay}->${record.timesPerDay}x/dia`
          : `${record.timesPerDay}x/dia`
        : `${record.timesPerDay}x/dia`,
    }
  })
          ...
          {enrichedHistory.map((record) => (
            <TableRow key={record.id}>
              <TableCell className="font-medium">
                {format(new Date(record.effectiveFrom), "dd MMM yyyy", {
                  locale: es,
                })}
              </TableCell>
              <TableCell>
                <Badge
                  ...
                  className="text-xs"
                >
                  {record.type === "rapid" ? "Rapida" : "Basal"}
                </Badge>
              </TableCell>
              <TableCell>
